...
const sectionLabelClass = 'px-2 mb-1 text-[12px] font-semibold tracking-[0.02em] text-slate-500 dark:text-slate-300';
...
export function NavMain({ sections = [] }) {
    ...
    // Auto-abre el menú que contiene la URL activa (solo al montar)
    useEffect(() => {
        setOpenMenus((prev) => {
            ...
            allItems.forEach((item) => {
                const isActive = (item.items ?? []).some((sub) => url === sub.url);
                if (isActive) {
                    activeStates[item.title] = true;
                }
            });
            return activeStates;
        });
        ...
        return (
            <SidebarMenuItem key={itemKey}>
                {collapsed ? (
                    subItems.length > 0 ? (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent
                                ...
                                sideOffset={4}
                            >
                                <DropdownMenuLabel className="text-sm font-normal text-slate-800 dark:text-slate-100">{formatLabel(item.title)}</DropdownMenuLabel>
                                <DropdownMenuSeparator />
                                {subItems.map((subItem, subIndex) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            <Link
                                                href={subItem.url}
                                                className="hover:bg-muted/40 flex w-full items-center gap-2 text-sm font-normal text-slate-800 dark:text-slate-100"
                                            >
                                                <SubIcon className="h-4 w-4" />
                                                <span>{formatLabel(subItem.title)}</span>
                                            </Link>
                                        </DropdownMenuItem>
                                    );
                                })}
                    ...
                    >
                        <Link href={item.url || '#'} aria-label={formatLabel(item.title)}>
                    ...
                    <>
                        <SidebarMenuButton onClick={() => toggleMenu(item.title)} className="text-sm font-normal text-slate-800 dark:text-slate-100">
                            <ItemIcon className="size-4" />
                            <span>{formatLabel(item.title)}</span>
                            <ChevronRight
                        ...
                        >
                            {subItems.map((subItem, subIndex) => {
                                const SubIcon = resolveIcon(subItem.icon);
                                return (
                                    <SidebarMenuSubItem key={`${itemKey}-sub-${subIndex}`}>
    ...
    return (
        <>
            {sections.map((section, sectionIndex) => (
                <SidebarGroup key={`${section.slug || section.label}-${sectionIndex}`}>
                    <SidebarGroupLabel className={sectionLabelClass}>
                        {formatLabel(section.label)}
                    </SidebarGroupLabel>
                    <SidebarMenu>
                        {(section.items || []).map((item, itemIndex) =>
                            renderMenuItem(item, `section-${sectionIndex}-${itemIndex}`)
                        )}
                    </SidebarMenu>
                </SidebarGroup>
            ))}
