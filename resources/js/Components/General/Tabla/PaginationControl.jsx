...
export function PaginationControl({ table }) {
    ...
    return (
        <div className="flex flex-col md:flex-row items-center justify-between mt-4 px-2 w-full gap-4">
            {/* Left Side: Info Text */}
            <div className="text-sm text-muted-foreground order-2 md:order-1">
                {totalItems > 0 ? (
                    <>Mostrando <strong>{start}-{end}</strong> de <strong>{totalItems}</strong> elementos</>
                ) : (
                    "Sin resultados"
                )}
            </div>
