    ...
    const permisosAcciones = (usuario) => {
        ...
        if (puede('ver')) {
            acciones.push({
                ...
                variant: 'primary',
                onClick: (item) => {
                    router.visit(`/dashboard/gestion-usuarios/${item.usu_id}`);
                },
            });
        }
        if (puede('editar')) {
        ...
        if (puede('desactivar')) {
            acciones.push({
                label: usuario.usu_activo ? 'Desactivar' : 'Activar',
                icon: usuario.usu_activo
                    ? iconoDe('desactivar')
                    : iconoDe('activar'),
                variant: 'danger',
                onClick: (item) => {
        ...
        { accessorKey: 'usu_tipo', header: 'Tipo' },
        {
            ...
            meta: {
                ...
                options: [
                    ...
                    { label: 'Bloqueado', value: 'Bloqueado', icon: UserLock },
                ],
            },
        },
        {
